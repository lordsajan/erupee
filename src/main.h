...
static const int COINBASE_MATURITY_ERP = 500;
...
public:
    ...
    {
        // erupee: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // erupee: get transaction coin age
...
public:
    ...
    // erupee: block signature - signed by coin base txout[0]'s owner
    ...
    // erupee: two types of block: proof-of-work or proof-of-stake
    ...
    // erupee: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // erupee: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // erupee: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // erupee: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // erupee: block index flags
    enum  
    {
